...
{
    Time new_time = Time(24 - hour, 60 - minute, 60 - second);
    return new_time;
}
...
auto Time::time_of_day() -> enum TIME_OF_DAY
{
    ...
    else if (hour > 11 && hour < 18) return dzien;
    else if (hour > 17 && hour < 23) return wieczor;
    else return noc;
}
...
auto main() -> int
{
    ...
    std::cout << "(test_2)\nPora dnia: " << test_2.to_string(test_2.time_of_day()) << "\n";
    ...
    if (test_2 != test) std::cout << "test_2 != test -> TRUE" << "\n";

    //zadanie - Sekundy do Północy

    std::cout << "test_2 - passed seconds: " << test_2.count_seconds() << "\n";
    std::cout << "test_2 - minutes to midnight: "  << result.time_to_midnight().to_string() << "\n";
